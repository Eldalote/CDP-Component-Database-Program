...
class Add_passive_window(Passive_single_window):
    ...
    def _create_window(self):
        ...
        self.gridrow += 1
        #Kicad foot
        Entry(self.window, width = 35, textvariable = self.ValueDict['KiCad Footprint']).grid(row = self.gridrow, column = 1)
        self.gridrow += 1
        #note generic
        ...
        self.window.bind("<F1>", self._button_add)
        self.window.bind("<F2>", self._button_add_close)
        self.window.bind("<F3>", self._button_close)
        self.window.bind("<F4>", self._button_octopart)
    ...
    def _button_octopart(self, event = None):
        ...
        print("Override failure _button_octopart(Add_passive_window)")
